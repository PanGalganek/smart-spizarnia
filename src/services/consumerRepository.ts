...
const consumers = collection(db, "consumers");

export async function listConsumers(): Promise<Consumer[]> {
  const snapshot = await getDocs(consumers);
...
export async function addConsumer(name: string) {
  ...
  await setDoc(doc(consumers, id), consumer);
  return consumer;
}
...
export async function removeConsumer(consumer: Consumer) {
  ...
  await deleteDoc(doc(consumers, consumer.id));
}
