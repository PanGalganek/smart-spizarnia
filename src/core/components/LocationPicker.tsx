...
export function LocationPicker({ value, onChange, label = "Lokalizacja", productType = "food" }: Props) {
  ...
  const pickerLayer = useNavigationLayer(`location-picker-${productType}`, "modal", { modal: "location-picker" });

  useEffect(() => { void listLocations(productType).then(setLocations); }, [productType]);

  function selectLocation(nextValue: string) {
    onChange(nextValue);
    pickerLayer.closeLayer();
  }

  return <View style={styles.field}>
    <Text style={styles.label}>{label}</Text>
    <Pressable onPress={pickerLayer.openLayer} style={styles.select}>
        ...
        <View style={styles.dialog}>
          ...
          <ScrollView style={styles.list} contentContainerStyle={styles.listContent}>
            {!locations.length ? <Text style={styles.empty}>Brak lokalizacji. Dodaj ją w kafelku Spiżarnia.</Text> : locations.map((item) => (
              <Pressable key={item} onPress={() => selectLocation(item)} style={[styles.option, displayedValue === item && styles.optionActive]}>
                <Text style={[styles.optionText, displayedValue === item && styles.white]}>{item}</Text>
                {displayedValue === item && <Text style={styles.white}>Wybrana</Text>}
              </Pressable>
            ))}
          </ScrollView>
...
const styles = StyleSheet.create({
  field: { width: "100%", gap: 9 },
  label: { fontWeight: "700", fontSize: 16 },
  select: { minHeight: 48, flexDirection: "row", alignItems: "center", justifyContent: "space-between", backgroundColor: colors.background, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12 },
  value: { color: colors.text, fontSize: 16, fontWeight: "700" },
  placeholder: { color: colors.muted, fontSize: 16 },
  arrow: { color: colors.primary, fontSize: 13 },
  ...
  noLocation: { alignItems: "center", borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 14 }
});
