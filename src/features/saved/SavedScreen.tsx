...
type ActionMode = "details" | "pantry" | "today";
    ...
    <ModuleScreen title="Zapisane">
      ...
      <Modal visible={!!selected} transparent animationType="fade" onRequestClose={() => setSelected(null)}>
        <View style={styles.backdrop}><View style={styles.modalCard}>
          <View style={styles.modalHeader}><View style={styles.modalHeading}>{actionMode !== "details" && <Pressable onPress={() => { setActionMode("details"); setError(""); }} style={styles.backAction}><Text style={styles.backActionText}>‹ Szczegóły</Text></Pressable>}<Text style={styles.modalTitle}>{selected?.name}</Text></View><Pressable onPress={() => setSelected(null)} style={styles.close}><Text style={styles.closeText}>Zamknij</Text></Pressable></View>
          ...
          <ScrollView style={styles.detailsScroll} contentContainerStyle={styles.details} showsVerticalScrollIndicator>
            ...
            {actionMode === "pantry" && <View style={styles.actionForm}>
              <Text style={styles.actionTitle}>Dodaj do spiżarni</Text>
              ...
              <Pressable disabled={busy} onPress={() => void addToPantry()} style={[styles.addButton, busy && styles.disabled]}><Text style={styles.white}>{busy ? "Dodawanie..." : "Potwierdź dodanie do spiżarni"}</Text></Pressable>
            </View>}
            {actionMode === "today" && <View style={styles.actionForm}>
              <Text style={styles.actionTitle}>Dodaj do dzisiejszego bilansu</Text>
              {selected && canUseWholePackage(selected) && <Pressable onPress={() => { setAmount("1"); setUnit("szt"); }} style={styles.packageButton}><Text style={styles.packageText}>Całe opakowanie: 1 szt. ({selected.packageAmount} {selected.packageUnit})</Text></Pressable>}
              <View style={styles.amountRow}><TextInput value={amount} onChangeText={setAmount} keyboardType="decimal-pad" style={styles.amountInput} />{(["g", "ml", "szt"] as Unit[]).map((value) => <Pressable key={value} onPress={() => setUnit(value)} style={[styles.unit, unit === value && styles.unitActive]}><Text style={unit === value ? styles.white : undefined}>{value}</Text></Pressable>)}</View>
              <ConsumerPicker value={consumer} onChange={setConsumer} label="Bilans kalorii dla" />
              <Pressable disabled={busy} onPress={() => void addToToday()} style={[styles.todayButton, busy && styles.disabled]}><Text style={styles.white}>{busy ? "Dodawanie..." : "Potwierdź dodanie do bilansu"}</Text></Pressable>
            </View>}
          </ScrollView>
